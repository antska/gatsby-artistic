...
(function (window) {
  ...
  DOM.nav = {
    ...
    forwardCtrl: DOM.content.querySelector('nav > .btn--nav-up'),
  };
  // Content slides.
  ...
  // The info text.
  DOM.infoText = DOM.content.querySelector('.overlay--info.room__subject');
  DOM.infoEyewitness = DOM.content.querySelector(
    '.overlay--info.menu__eyewitnesses',
  );
  DOM.infoProvenance = DOM.content.querySelector(
    '.overlay--info.menu__provenance',
  );

  DOM.walls = [
    DOM.scroller.querySelector('.room__side--left'),
    DOM.scroller.querySelector('.room__side--right'),
    DOM.scroller.querySelector('.room__side--back'),
  ];

  DOM.goBack = DOM.content.querySelector('.btn--go-back');
  DOM.roomDesc = DOM.content.querySelector('.room-desc');
  ...
  function initEvents() {
    ...
    DOM.goBack.addEventListener('click', goBack);

    DOM.walls.forEach((wall) => {
  ...
  function goBack() {
    isInFront = false;
    DOM.infoCtrl.style.visibility = 'visible';
    DOM.menuCtrl.style.visibility = 'visible';
    DOM.nav.leftCtrl.style.visibility = 'visible';
    DOM.nav.rightCtrl.style.visibility = 'visible';

    DOM.roomDesc.removeAttribute('hidden');
    DOM.goBack.setAttribute('hidden', '');

    DOM.walls.forEach((wall) => {
      wall.style.transform = '';
      wall.style.justifyContent = '';
      wall.style.cursor = 'zoom-in';
      wall.classList.remove('selected__room');
      wall.querySelector('.room__img--desc').style.width = '30%';
      if (
        wall.classList.contains('room__side--left') ||
        wall.classList.contains('room__side--right')
      ) {
        wall.style.width = '3000px';
      }
    });
  }

  function zoomInWall(wall) {
    if (!isInFront) {
      wall.style.width = '100vw';
      wall.style.transform = 'rotate3d(0, 1, 0, 0)';
      wall.style.justifyContent = 'flex-start';
      wall.style.cursor = 'auto';
      wall.classList.add('selected__room');
      wall.querySelector('.room__img--desc').style.width = 'auto';

      DOM.infoCtrl.style.visibility = 'hidden';
      DOM.menuCtrl.style.visibility = 'hidden';
      DOM.nav.leftCtrl.style.visibility = 'hidden';
      DOM.nav.rightCtrl.style.visibility = 'hidden';
      DOM.roomDesc.setAttribute('hidden', '');
      DOM.goBack.removeAttribute('hidden');
      DOM.goBack.style.opacity = 1;
  ...
  function closeMenu(clickedOnLink = false) {
    // Button becomes menu.
    ...
    // back to room view:
    if (!clickedOnLink) {
      move({ transform: resetTransform, stopTransition: true }).then(
        function () {
  ...
  function showInfo(trigger = null) {
    // Button becomes cross.
    ...
    //   translateZ: 0,
    // });
    if (trigger?.textContent === 'Eyewitnesses') {
      infoElement = DOM.infoEyewitness;
    } else if (trigger?.textContent === 'Provenance') {
      infoElement = DOM.infoProvenance;
    } else {
      infoElement = DOM.infoText;
    }

    // Animate info text and overlay.
    anime.remove(infoElement);
    var animeInfoOpts = {
      targets: infoElement,
      duration: 1000,
      delay: function (t, i) {
      ...
      begin: function () {
        infoElement.classList.add('overlay--active');
      },
    };
    anime(animeInfoOpts);
  }
  ...
  function closeInfo() {
    ...
    // showSlide(100);
    // back to room view:
    move({ transform: resetTransform, stopTransition: true }).then(function () {
      initTilt();
    });
    ...
    var animeInfoOpts = {
      ...
      complete: function () {
        infoElement.classList.remove('overlay--active');
      },
    };
    anime(animeInfoOpts);
  }
